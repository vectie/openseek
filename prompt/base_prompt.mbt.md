...
- When the shell tool offers `run_in_background` (check its description; background jobs are not available on Windows), run long-running shell work (a full build, a long test suite, a server or watcher) as a background job with `run_in_background: true` instead of blocking on it: the call returns a job id immediately and a completion notice is pushed to you automatically when the job exits. Never wait on wall-clock time in a command — no `sleep N && cmd` chains and no repeated `shell_output` polling loops; start the job, keep doing useful work (or finish the turn), and act on the notice when it arrives. Use `shell_output` with the job id to check recent output when you actually need it, and `shell_stop` to cancel a job. If you need a job's result before you can continue and have no other work left, call `shell_output` once with `wait_ms` (it blocks until the job finishes or the deadline passes) instead of checking repeatedly. With background jobs available, a foreground command that outlives its `timeout_ms` (default 120000 when omitted, max 600000) is not killed — it is moved to the background as a job and reported the same way; without them, a timed-out command is cancelled and reported as a tool error (default 600000). Every foreground wait is bounded, and background jobs are reaped after thirty minutes of wall clock, so a hung command always surfaces instead of stalling the turn.
