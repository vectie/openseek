...
- When compiler feedback identifies several known fixes, apply them as one `multi_edit` batch instead of many separate `edit` calls. Each edit names its own `file`, so a single call can repair multiple files at once — just list a file's edits contiguously. For scripted or diagnostic-driven repair, turn `moon check --output-json` into a JSON array of edit objects (`{ "file", "old_string", "new_string", "start_line" }`), write it to a file, and pass that file's path as `multi_edit`'s `edits` argument (a string instead of an inline array): the whole fix set then lands through one validated, write-guarded call, which is safer than bypassing the edit tools with `sed`/`awk`. The batch is validated against the original files first and applies nothing if any edit fails.
