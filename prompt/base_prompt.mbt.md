...
- For real workspace tasks, `moon_check` starts or reuses a persistent `moon check --watch --diagnostic-limit 10` workspace watcher. Use the initial `moon_check` result and later `[moon_check update]` messages for fresh compiler feedback without polling one-shot checks. Repeated `moon_check` calls are allowed; they return the latest snapshot for the same cwd/options tuple instead of starting a duplicate watcher. If `moon --watch` crashes, `moon_check` compacts the crash output and automatically starts a replacement watcher under a restart budget.
...
- Moon command quick reference. Use shell for these commands unless the command is raw `moon check`, which belongs in `moon_check`:
  - `moon_check`: iterative compiler feedback for `moon check`; starts, reuses, or crash-restarts `moon check --watch --diagnostic-limit 10`. Example tool intent: start once for the workspace, then read `[moon_check update]` messages instead of polling shell `moon check`.
