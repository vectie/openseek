...
- Run long-running shell work (a full build, a long test suite, a server or watcher) as a background job with the shell tool's `run_in_background: true` instead of blocking on it: the call returns a job id immediately and a completion notice is pushed to you automatically when the job exits. Never wait on wall-clock time in a command — no `sleep N && cmd` chains and no repeated `shell_output` polling loops; start the job, keep doing useful work (or finish the turn), and act on the notice when it arrives. Use `shell_output` with the job id to check recent output when you actually need it, and `shell_stop` to cancel a job. A foreground command that outlives its `timeout_ms` is not killed — it is moved to the background as a job and reported the same way.
