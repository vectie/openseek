...
- For native CLIs, import `moonbitlang/core/env`, `moonbitlang/async/fs`, `moonbitlang/async/stdio`, and `moonbitlang/async/io`; use `@env.args()` for arguments, `@fs.read_file(path).text()` for file input, and `@stdio.stdin.read_all().text()` for `--stdin`. Validate stdin mode through shell piping, for example `printf 'a.b = 1\n' | moon run --target native cmd/tomljson -- --stdin`. Do not use `/dev/stdin` or C FFI for ordinary stdin handling. In native `moon run`, `@env.args()[0]` can be the generated native/C entry path rather than the first user argument; inspect args with a tiny smoke test or drop that executable path before reading `schema.json`, `input.toml`, or other user files.
...
- Current MoonBit uses `moon.mod`; `moon.mod.json` is legacy. For new projects, create `moon.mod`, not `moon.mod.json`. Only preserve `moon.mod.json` when an existing project is deliberately still in legacy mode. After editing `moon.mod`, `moon.pkg`, or package imports, immediately start or inspect `moon_check` before continuing.
...
- Avoid one large generated file or one huge write. Add focused files or blocks, call `moon_check` once near the start of longer iterative MoonBit work, and only continue once the current package checks. Avoid repeatedly polling one-shot `moon check` while a watcher is available.
- For real workspace tasks, `moon_check` starts or reuses a persistent `moon check --watch --output-json --diagnostic-limit 10` watcher. Use the initial `moon_check` result and later `[moon_check update]` messages for fresh compiler feedback without polling one-shot checks. Repeated `moon_check` calls are allowed; they return the latest snapshot for the same cwd/path/options tuple instead of starting a duplicate watcher. If `moon --watch` crashes, `moon_check` compacts the crash output and automatically starts a replacement watcher under a restart budget.
...
- Moon command quick reference. Use shell for these commands unless the command is raw `moon check`, which belongs in `moon_check`:
  - `moon_check`: iterative compiler feedback for `moon check`; starts, reuses, or crash-restarts `moon check --watch --output-json --diagnostic-limit 10`. Example tool intent: start once for the workspace, then read `[moon_check update]` messages instead of polling shell `moon check`.
